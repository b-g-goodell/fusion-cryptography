...
def test_fusion_keygen(example_params):
    for secpar in [128, 256]:
        ...
        assert isinstance(otsk.left_sk, GeneralMatrix)
        assert isinstance(otsk.rght_sk, GeneralMatrix)

        inv_left_sk_hat: GeneralMatrix = GeneralMatrix(
            matrix=[[transform(f) for f in row] for row in otsk.left_sk.matrix]
        )
        inv_right_sk_hat: GeneralMatrix = GeneralMatrix(
            matrix=[[transform(f) for f in row] for row in otsk.rght_sk.matrix]
        )
        ...
        assert params.public_challenge * otsk.left_sk == otvk.left_vk_hat
        assert params.public_challenge * otsk.rght_sk == otvk.right_vk_hat
